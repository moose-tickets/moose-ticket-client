...
export default function Splash() {
  ...
  useEffect(() => {
    const initializeAndNavigate = async () => {
      try {
        // Initialize the app (this will hydrate auth state from storage)
        await dispatch(initializeApp());
        
        // Check onboarding status
        const onboardingSeen = await AsyncStorage.getItem("onboarding_shown");
        
        // Determine navigation route based on current auth state
        // No need to fetch user data - it's already in Redux from login or hydration
        let route: string;
        let screen: string;
  ...
  }, [navigation, dispatch]); // Removed isAuthenticated and user from dependencies to prevent re-renders
  return (
    <LinearGradient
